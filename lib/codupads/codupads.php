...
if ( !class_exists('CodupAds') ) :
    ...
    class CodupAds {
        ...
        function add_style_scripts() {
            $pluginConfig = array(
                  "pluginName" => CODUP_PR_PLUGIN_NAME,
                  "pluginVersion" => CODUP_PR_PLUGIN_VER,
                  "pageSlug" => CODUP_PR_PAGE_SLUG,
                  "wpVersion" => get_bloginfo('version'),
                  "wcVersion" => $this->cwrf_get_woo_version_number()
            );
            wp_enqueue_script(CODUP_PR_PLUGIN_NAME, plugin_dir_url(__FILE__) . 'scripts/adscript.js', array('jquery'), CODUP_PR_PLUGIN_VER, false);
            wp_enqueue_style(CODUP_PR_PLUGIN_NAME . '- codupads-styles', plugin_dir_url(__FILE__) . 'styles/style.css', array(), CODUP_PR_PLUGIN_VER, 'all');
            wp_localize_script(CODUP_PR_PLUGIN_NAME, 'PluginConfig', $pluginConfig);
        }
